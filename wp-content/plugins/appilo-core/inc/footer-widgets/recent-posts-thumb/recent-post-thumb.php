<?php
/**
 * Appilo_Recent_Posts widget class
 *
 * @since 1.0
 */
class Appilo_Recent_Posts extends WP_Widget {
	...
	function __construct() {
		$this->defaults[ 'author_label' ]		= __( 'By', 'appilo_recent_posts' );
		$this->defaults[ 'category_ids' ]		= array( 0 ); // selected categories
		$this->defaults[ 'category_label' ]		= _x( 'In', 'In {categories}', 'appilo_recent_posts' ); // label for category list
		...
		$this->defaults[ 'nav_label' ]			= __( 'Recent Posts With Thumbnails', 'appilo_recent_posts' ); // ARIA label of the navigation list
		$this->defaults[ 'number_posts' ]		= 5; // number of posts to show in the widget
		$this->defaults[ 'plugin_slug' ]		= 'appilo_recent_posts'; // identifier of this plugin for WP
		...
		$widget_desc = 'Appilo Recent Post with Thumbnail';
        $widget_name  = 'Appilo Recent Posts';
		...
		$widget_name = __( 'Recent Posts With Thumbnails', 'appilo_recent_posts' );
		$widget_desc = __( 'List of your site&#8217;s most recent posts, with clickable title and thumbnails.', 'appilo_recent_posts' );
	...
	public function load_plugin_textdomain() {
		...
		load_plugin_textdomain( 'appilo_recent_posts', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	...
	private function get_the_categories ( $id ) {
		...
		if ( is_wp_error( $terms ) ) {
			return __( 'Error on listing categories', 'appilo_recent_posts' );
		}
...
function register_recent_posts_widget_with_thumbnails () {
	register_widget( 'Appilo_Recent_Posts' );
}
