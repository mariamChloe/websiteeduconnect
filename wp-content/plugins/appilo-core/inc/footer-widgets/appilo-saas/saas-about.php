<?php
// Adds widget: SaaS About
class Saasabout_Widget extends WP_Widget {

    function __construct() {
        parent::__construct(
            'saasabout_widget',
            esc_html__( 'SaaS About', 'appilo' ),
            array( 'description' => esc_html__( 'SaaS footer about', 'appilo' ), ) // Args
        );
    ...
    public function widget( $args, $instance ) {
        ...
        $saas_logo_id = apply_filters( 'saas_footer_logo_image_uri', $instance[ 'saas_footer_logo' ] );
        $saas_about_logo = wp_get_attachment_url($saas_logo_id);
        ...
        echo'<div class="footer_logo">
                <a href="'.esc_url( home_url( '/' ) ).'"><img src="'.esc_url($saas_about_logo).'" alt></a>
                </div>
                <div class="footer_text">
                    '.$instance['saas_footer_about'].'
                </div>
                <div class="footer_social ul-li clearfix">
                    <ul>';
        if ( ! empty( $instance['saas_fb_link'] ) || ! empty( $instance['saas_fb_link'] ) ) {
            echo '<li><a href="'.$instance['saas_fb_link'].'"><i class="fab fa-facebook-f "></i></a></li>';
        }
        if ( ! empty( $instance['saas_be_link'] ) || ! empty( $instance['saas_be_link'] ) ) {
            echo '<li><a href="'.$instance['saas_be_link'].'"><i class="fab fa-behance"></i></a></li>';
        }
        if ( ! empty( $instance['saas_ln_link'] ) || ! empty( $instance['saas_ln_link'] ) ) {
            echo '<li><a href="'.$instance['saas_ln_link'].'"><i class="fab fa-linkedin"></i></a></li>';
        }
        if ( ! empty( $instance['saas_tw_link'] ) || ! empty( $instance['saas_tw_link'] ) ) {
            echo '<li><a href="'.$instance['saas_tw_link'].'"><i class="fab fa-twitter"></i></a></li>';
        }
        echo'</ul>
            </div>';
...
function register_saasabout_widget() {
    register_widget( 'Saasabout_Widget' );
}
add_action( 'widgets_init', 'register_saasabout_widget' );